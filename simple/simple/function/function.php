...
{
    $title = '【' . date("H:i:s") . '】————————————————————————————————————————————————————————————' . "\r\n";
    $fileName = './simple/log/debug/' . date("Y-m-d") . '.txt';
    if (is_array($content)) {
        $data = 'array(' . "\r\n";
...
{
    $title = '【' . date("H:i:s") . '】————————————————————————————————————————————————————————————' . "\r\n";
    $fileName = './simple/log/sql/' . date("Y-m-d") . '.txt';
    if (is_array($content)) {
        $data = 'array(' . "\r\n";
        foreach ($content as $k => $v) {
            $data .= '[' . $k . ']=>' . $v . "\r\n";
...
{
    $upload = new \simple\library\core\upload(array('jpg', 'jpeg', 'png'));
 ...
 * @return int|string
 */
function ftpUpload($farr)
{ //传递参数为$_FILES,用户form提交上传的文件变量
    $ftp_server = "122.14.220.202";  //ftp服务端ip地址
    $ftp_user_name = "jxkjapp"; //ftp用户名
    $ftp_user_pass = "PIMaHjgLxwT2SbT8iSScmz0Uf"; //ftp密码
    $conn_id = ftp_connect($ftp_server);
    $file = $farr['img']['tmp_name'];
    $arr = explode('.', $farr['img']['name']);
    $arr = strtolower($arr[count($arr) - 1]);
    if (!in_array($arr, array('jpg', 'gif', 'png', 'jpeg'))) {
        return '';
    }
    $remote_file = time() . '.' . $arr; //随机文件名,防止重复
    $login_result = ftp_login($conn_id, $ftp_user_name, $ftp_user_pass);
    ftp_chdir($conn_id, 'upload');
    $exist_dir = ftp_nlist($conn_id, ftp_pwd($conn_id));
    $date = date("Y-m-d");
    $dir_name = '/upload/' . $date;
    //按日期创建文件夹
    ...
    if (ftp_put($conn_id, $remote_file, $file, FTP_BINARY)) {
        ftp_close($conn_id);
        return $dir_name . '/' . $remote_file; //返回文件路径和名称,记录到数据库中以便调用
    } else {
        return '';
    }
}
