...
use simple\traits\error;
use simple\traits\verify;
use simple\traits\ajax;
use simple\traits\view;
...
class model
{
    ...
    public static function getInstance($config)
    {
        ...
        if (!static::$instance instanceof self) {
            static::$instance = new $cName($config);
        }
...
<!--第二步：添加如下 HTML 代码-->
<table id="datatables" class="display responsive nowrap cell-border" style='table-layout：fixed;'>
    <thead>
        <tr>
            <th>提示</th>
            <th>内容</th>
...
top;
            for ($i = 1; $i < count($e->getTrace()); $i++) {
                echo '<tr>';
                echo "<th>错误位置{$i}</th>";
                echo "<td>{$e->getTrace()[$i]['file']}（第{$e->getTrace()[$i]['line']}行）</td>";
                echo '</tr>';
            }
            echo <<<bottom
    </tbody>
